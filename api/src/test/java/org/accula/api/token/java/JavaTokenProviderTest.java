...
class JavaTokenProviderTest {
    ...
    @Test
    void test() {
        final var f1 = new FileEntity<>("ref1", "Cell.java", SuffixTreeCloneDetectorTest.F1, LineSet.all());
        final var f2 = new FileEntity<>("ref2", "Cell.java", SuffixTreeCloneDetectorTest.F2, LineSet.all());
        ...
        StepVerifier.create(tokenProvider.tokensByMethods(Flux.just(f1, f2, f3))
                .collectList())
                .expectNextMatches(methods -> methods.size() == 6) // Non-empty methods
                .verifyComplete();
    }
}
