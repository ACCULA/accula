...
class GithubWebhookRouterTest {
    ...
    void testPushKeepFilesSyncedEnabled() {
        final var files = List.of(Project.Conf.KEEP_FILE_LIST_SYNCED_WITH_GIT, "f1", "f2");
        when(projectRepo.idByRepoId(any()))
            .thenReturn(Mono.just(1L));
        when(projectRepo.confById(any()))
            .thenReturn(Mono.just(Project.Conf.DEFAULT.withExcludedFiles(List.of(Project.Conf.KEEP_FILE_LIST_SYNCED_WITH_GIT))));
