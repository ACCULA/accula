...
class CloneDetectorTest {
    ...
    void test() {
        ...
        var commitSnapshot1 = Snapshot.builder().commit(Commit.shaOnly("sha1")).branch("branch1").repo(repo1).build();
        var commitSnapshot2 = Snapshot.builder().commit(Commit.shaOnly("sha2")).branch("branch2").repo(repo2).build();
