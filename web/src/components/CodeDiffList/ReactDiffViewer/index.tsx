// Fork of https://github.com/praneshr/react-diff-viewer/tree/v3.0.0
...
class DiffViewer extends React.Component<ReactDiffViewerProps, ReactDiffViewerState> {
  ...
  private renderSplitView = ({ left, right }: LineInformation, index: number): JSX.Element => {
    return (
      <tr key={index} className={this.styles.line}>
        {this.renderLine(
          this.props.leftOffset + index,
          left.type,
          LineNumberPrefix.LEFT,
          left.value
        )}
        {this.renderLine(
          this.props.rightOffset + index,
          right.type,
  ...
  private renderSkippedLineIndicator = (
    ...
    const content = (
      <td>
        <a href="/#" onClick={this.onBlockClickProxy(blockNumber)} tabIndex={0}>
          {message}
        </a>
      </td>
    )
    return (
      <tr key={`${leftBlockLineNumber}-${rightBlockLineNumber}`} className={this.styles.codeFold}>
        {!this.props.hideLineNumbers && <td className={this.styles.codeFoldGutter} />}
